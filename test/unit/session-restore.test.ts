...
describe('rendered-view session restoration', () => {
  ...
  it('generates case-insensitive stable keys for the same PR', () => {
    ...
    expect(
      sessionKeyFromMdprParts({ ...PARTS, project: 'Other Project' }),
    ).to.not.equal(sessionKeyFromMdprParts(PARTS));
    expect(
      sessionKeyFromMdprParts({ ...PARTS, pullRequestId: 62 }),
    ).to.not.equal(sessionKeyFromMdprParts(PARTS));
  });
  ...
  it('rejects sessions with different organization, project, or repository', () => {
    ...
    expect(
      sessionMatchesMdprParts(
        makeSession({ ...PARTS, organization: 'Fabrikam' }),
        PARTS,
      ),
    ).to.equal(false);
    expect(
      sessionMatchesMdprParts(
        makeSession({ ...PARTS, project: 'Other Project' }),
        PARTS,
      ),
    ).to.equal(false);
    expect(
