...
describe('recent PR cache', () => {
  it('defines the persisted-state key and default cache limit', () => {
    expect(RECENT_PULL_REQUESTS_STATE_KEY).to.equal(
      'markdownPrReview.recentPullRequests',
    );
  ...
  it('creates a persisted recent item from a pull request', () => {
    ...
    const item = recentPullRequestFromPullRequest(
      pr,
  ...
  it('deduplicates reopened PRs and moves the latest entry to the top', () => {
    ...
    expect(result.map((item) => item.title)).to.deep.equal([
      'Updated title',
      'Other PR',
    ]);
  ...
  it('keeps only the requested number of recent PRs', () => {
    ...
    expect(result.map((item) => item.ref.pullRequestId)).to.deep.equal([
      4, 1, 2,
    ]);
  });
  ...
  it('uses case-insensitive organization/project/repository identity', () => {
    ...
    expect(recentPullRequestKey(lower.ref)).to.equal(
      recentPullRequestKey(upper.ref),
    );
  });
  ...
  it('keeps repository id and repository name identities distinct', () => {
    ...
    expect(recentPullRequestKey(withName.ref)).to.not.equal(
      recentPullRequestKey(withId.ref),
    );
  });
  ...
  it('returns an empty recent list for non-array stored state', () => {
    ...
    expect(
      parseStoredRecentPullRequests({ recent: [makeRecent(42)] }),
    ).to.deep.equal([]);
  });
  ...
  it('drops entries with incomplete metadata or invalid refs', () => {
    ...
    expect(
      parseStoredRecentPullRequests([valid, ...invalidEntries]),
    ).to.deep.equal([valid]);
  });
});
...
describe('isPullRequestRef guard', () => {
  ...
  it('rejects the file-tree placeholder node passed by the view/title button', () => {
    const placeholder = {
      kind: 'message',
      label:
        'No active PR. Run "Markdown PR Review: Open Pull Request…" to start.',
...
function makeRecent(
  ...
  return recentPullRequestFromPullRequest(
    makePullRequest(pullRequestId, title),
    openedAt,
  );
}
