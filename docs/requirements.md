...
**REQ-CORE-008** *(v0.5)*: The extension SHALL register a command `markdownPrReview.openPullRequestForCurrentBranch` which discovers active Azure DevOps pull requests whose source branch is the current branch of the active workspace repository and lets the reviewer select one to open in the existing review surface (REQ-CORE-001).

- AC-1: When invoked, the command determines the current git branch and resolves the ADO organization, project, and repository from the active repository's remote, then queries ADO for pull requests whose `sourceRefName` is `refs/heads/{branch}` and whose status is `active`.
- AC-2: The results are presented in a Quick Pick — shown even when exactly one match exists — and selecting an entry opens that PR through the existing open pipeline (REQ-CORE-001); the pre-existing open-by-URL/ID entry point is unchanged.
- AC-3: When zero active PRs match the current branch, the extension informs the user and notes that PRs whose source branch lives in a fork are not discovered in v1.
- AC-4: When the workspace is not a git repository, the built-in Git extension is unavailable, HEAD is detached (no branch), or the remote does not resolve to an Azure DevOps Services repository, the extension surfaces a distinct, actionable message for each case (REQ-ERR-001) and MUST NOT throw.
- AC-5: The command reuses the existing ADO authentication (REQ-AUTH-001) and REST client; it introduces no new authentication path and no new secret logging.
...
**CON-007**: The extension MUST NOT bundle or invoke any external git binary. Pull-request and file data are obtained from the ADO REST API. As a narrow, read-only exception (v0.5, REQ-CORE-008), the extension MAY read the current branch name and remote URLs of the active workspace repository through the built-in VS Code Git extension API (`vscode.git`) — an in-process API, not an external git binary — solely to discover the pull request for the current branch. It MUST NOT write to the repository or workspace (CON-006).
...
**ASM-010**: The built-in VS Code Git extension (`vscode.git`) is present, enabled, and — for the active workspace repository — reaches its `initialized` state, exposing the current branch (`HEAD`) and remotes via `getAPI(1)`. **If wrong**: REQ-CORE-008 cannot determine the current branch or remote; the command surfaces an actionable message per REQ-CORE-008 AC-4 and the reviewer falls back to the open-by-URL/ID entry point (REQ-CORE-001).
...
| 0.4 | 2026-07-09 | Active-Branch PR Discovery (user sign-off) | Added REQ-CORE-008 (v0.5) for the `markdownPrReview.openPullRequestForCurrentBranch` command — discovers active PRs whose source branch is the current branch and opens the chosen one in the existing review surface. Added a narrow read-only-local-git carve-out to CON-007 (read current branch + remotes via the built-in `vscode.git` API; no git binary, no workspace writes). Added ASM-010 (VS Code Git extension availability). No requirement renumbering; no cross-references broken. |
