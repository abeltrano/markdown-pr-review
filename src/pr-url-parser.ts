...
const VS_URL_RE =
  /^https?:\/\/([a-z0-9-]+)\.visualstudio\.com\/([^/]+)\/_git\/([^/]+)\/pullrequest\/(\d+)(?:[/?#].*)?$/i;
const ADO_URL_RE =
  /^https?:\/\/dev\.azure\.com\/([a-z0-9-]+)\/([^/]+)\/_git\/([^/]+)\/pullrequest\/(\d+)(?:[/?#].*)?$/i;
const BARE_ID_RE = /^\d+$/;
...
export function parsePullRequestInput(
  ...
  if (BARE_ID_RE.test(input)) {
    ...
    if (!Number.isFinite(id) || id <= 0) {
      return error(
        'invalid-pr-id',
        `PR id is not a positive integer: ${input}`,
        input,
      );
    }
    return {
...
function makeRef(
  ...
  if (!Number.isFinite(id) || id <= 0) {
    return error(
      'invalid-pr-id',
      `PR id is not a positive integer: ${idStr}`,
      input,
    );
  }
  return {
