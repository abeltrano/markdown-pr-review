...
export class HttpAdoClient implements AdoClient {
  ...
  async getPullRequest(ref: PullRequestRef): Promise<PullRequest> {
    ...
    return {
      ...
      lastMergeSourceCommit: {
        commitId: raw.lastMergeSourceCommit?.commitId ?? '',
      },
  ...
  async getChangedFiles(
    ...
    for (const entry of raw.changeEntries ?? []) {
      ...
      const changeType: ChangedFile['changeType'] = ct.includes('add')
        ? 'add'
        : ct.includes('delete')
          ? 'delete'
          : ct.includes('rename')
    ...
    while (attempt < maxAttempts) {
      ...
      try {
        ...
        if (
          ...
          this.log.info(
            'No cached session; prompting for interactive sign-in.',
            { url },
          );
      ...
      this.log.info(
        `${method} ${url}`,
        redactAuthHeaders({ attempt, method, url }),
      );
      ...
      if (response.status === 401) {
        ...
        if (auth401Retries === 0) {
          ...
          this.log.warn('401 received; retrying once with interactive auth.', {
            url,
          });
          continue;
        }
        throw new AdoRestError(
          ...
          url,
          safeBody,
          'Unauthorized — token may be invalid or scope rejected.',
        );
      }
      if (response.status === 403) {
        throw new AdoRestError(
          ...
          url,
          safeBody,
          'Forbidden — your account may not have permission for this repository.',
        );
      }
      if (response.status === 404) {
        throw new AdoRestError(
          ...
          url,
          safeBody,
          'Not found — check the PR URL or your access.',
        );
      }
  ...
  private expectOrg(_repoId: string): string {
    throw new Error(
      'HttpAdoClient.getFileContent requires the caller to pass the org explicitly. Use getFileContentByRef instead.',
    );
  }
...
function toThread(raw: AdoRawThread): Thread {
  return {
    ...
    comments: (raw.comments ?? []).map((c) => ({
      ...
      commentType:
        (c.commentType as 'text' | 'codeChange' | 'system') ?? 'text',
    })),
