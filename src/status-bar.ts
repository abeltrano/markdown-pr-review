...
export class StatusBarController implements vscode.Disposable {
  ...
  constructor() {
    ...
    this.disposables.push(
      vscode.commands.registerCommand(FOCUS_RENDERED_VIEW_COMMAND, () =>
        this.focusActiveRenderedView(),
      ),
    );
  }
  ...
  private focusActiveRenderedView(): void {
    ...
    const firstOpenedUri = this.currentSession.openedEditors
      .keys()
      .next().value;
    if (!firstOpenedUri) {
      void vscode.window.showInformationMessage(
        'No Markdown PR Review editor is currently open.',
      );
      return;
    }
