...
export class FileTreeProvider implements vscode.TreeDataProvider<FileNode> {
  ...
  constructor(
    ...
    this.markdownOnly = context.globalState.get<boolean>(
      MARKDOWN_ONLY_KEY,
      false,
    );
  ...
  getTreeItem(node: FileNode): vscode.TreeItem {
    ...
    if (node.kind === 'pullRequest') {
      ...
      const expanded =
        node.active || this.sessionManager.getOpenSessions().length === 1
    ...
    const uri = buildMdprUri(
      {
        ...node.session.pr.ref,
        repositoryId: node.session.pr.ref.repositoryId,
      },
      file.filePath,
    );
    item.command = {
        ...
        return [
          {
            ...
            tooltip:
              'Turn off the markdown-only filter to show all changed files.',
          },
        ];
      }
