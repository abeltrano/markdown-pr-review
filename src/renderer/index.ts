...
function injectDiffAttributes(tokens: Token[], annotations: DiffAnnotation[]): void {
    ...
    for (const token of tokens) {
        if (token.map) {
            ...
            const match = annotations.find(a => rangesOverlap(a.headLineStart, a.headLineEnd, start, end));
            if (match) {
