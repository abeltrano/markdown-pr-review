...
export function mapSelection(input: MapSelectionInput): MapSelectionResult {
  ...
  if (selection.spansMultipleBlocks) {
    ...
    const unionStart = clampLine(
      Math.min(...ranges.map((r) => r.start)),
      lines.length,
    );
    const unionEnd = clampLine(
  ...
  const disamb = disambiguate({
    matches,
    approxStart: normalizedBefore.length,
  });
  ...
  const rightFileStart: LineOffset = {
    line: startMap.line,
    offset: startMap.offset,
  };
  ...
  const rightFileEnd: LineOffset = {
    line: endMap.line,
    offset: endMap.offset + 1,
  };
  return {
    mode: 'precise',
    rightFileStart,
...
function coarse(
  ...
  return {
    ...
    rightFileEnd: {
      line: blockEnd,
      offset: Math.max(1, endLineText.length + 1),
    },
